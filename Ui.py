...
def calculate_damage(stats, bleed_status):
    ...
    target_resist = min(100, max(0, float(stats[10]))) / 100
    ...
    # Модификаторы урона
    total_modifier = min(stats[11], 75) if len(stats) > 11 else 0
...
root.geometry("350x700")  # Увеличили ширину для лучшего отображения
...
bleed_check.place(x=180, y=20 + 12*35)

# Кнопка расчета
calculate_btn = ttk.Button(root, text="Calculate Damage", command=calc_damage)
calculate_btn.place(x=100, y=20 + 13*35, width=150)
...
result_label.place(x=20, y=20 + 14*35, width=310, height=170)
